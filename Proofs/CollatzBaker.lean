...
abbrev CollatzHelixLFunction (m : ℕ) (s : ℂ) : ℂ :=
  GRHStub.helixPositionLFunction step_offset m s

abbrev collatz_projection_residue (m : ℕ) : ℝ :=
  GRHStub.projectionResidue CollatzHelixLFunction m

abbrev CollatzLFunction (n : ℕ) (s : ℂ) : ℂ :=
  GRHStub.helixOrbitLFunction collatz_projection_residue Collatz.orbit n s

abbrev accumulated_projection_loss (n k : ℕ) : ℝ :=
  GRHStub.accumulatedProjectionLoss collatz_projection_residue Collatz.orbit n k

/-- The Collatz residue is exactly the local GRH-style residue attached to
    the step-offset model, so the generic GRH residue bridge applies. -/
theorem collatz_projection_residue_matches_offset :
  ∀ m : ℕ, collatz_projection_residue m = step_offset m := by
  intro m
  have h_err : GRHStub.projectionLossError step_offset m = 0 :=
    GRHStub.projection_residue_recovers_baseOffset step_offset m
  have h_err' : collatz_projection_residue m - step_offset m = 0 := by
    simpa [GRHStub.projectionLossError, collatz_projection_residue, CollatzHelixLFunction] using h_err
  linarith

/-- With the GRH projection bridge in place, accumulated residue matches the
    geometric accumulated offset along the orbit. -/
theorem accumulated_projection_loss_eq_accumulated_offset (n k : ℕ) :
    accumulated_projection_loss n k = accumulated_offset n k := by
  simp [accumulated_projection_loss, accumulated_offset, GRHStub.accumulatedProjectionLoss,
    collatz_projection_residue_matches_offset]

/-- Generic and Collatz divergence predicates coincide definitionally. -/
theorem collatz_diverges_eq_orbitEscapes (n : ℕ) :
    Collatz.diverges n = GRHStub.OrbitEscapes Collatz.orbit n := rfl

/-- Once projection-loss separation reaches one unit, the orbit cannot return
    to the same integer endpoint. -/
theorem orbit_injective_of_projection_loss
    (n j k : Nat)
    (h_res : accumulated_projection_loss n k ≥ accumulated_projection_loss n j + 1) :
    Collatz.orbit n k ≠ Collatz.orbit n j := by
  have h_gap : GRHStub.endpointProjectionGap collatz_projection_residue Collatz.orbit n j k ≥ 1 := by
    simp [GRHStub.endpointProjectionGap] at *
    linarith
  exact GRHStub.endpoint_separation_of_projection_loss collatz_projection_residue Collatz.orbit n j k h_gap

/-- An escaping Collatz orbit eventually accrues at least one full unit of
    projection loss along some segment. -/
theorem escape_forces_projection_rollover
    (n : Nat) (_h_n : n > 4) (h_div : Collatz.diverges n) :
    ∃ j k, j < k ∧ accumulated_projection_loss n k ≥ accumulated_projection_loss n j + 1 := by
  have h_div' : GRHStub.OrbitEscapes Collatz.orbit n := by
    simpa [collatz_diverges_eq_orbitEscapes n] using h_div
  obtain ⟨j, k, hjk, h_gap⟩ :=
    GRHStub.escape_forces_projection_rollover collatz_projection_residue Collatz.orbit n h_div'
  refine ⟨j, k, hjk, ?_⟩
  simp [GRHStub.endpointProjectionGap] at *
  linarith

/-- Once the accumulated projection loss crosses one unit, the Collatz orbit
    can no longer sustain divergence. -/
theorem projection_rollover_blocks_divergence
    (n j k : Nat) (h_jk : j < k)
    (h_res : accumulated_projection_loss n k ≥ accumulated_projection_loss n j + 1) :
    ¬ Collatz.diverges n := by
  intro h_div
  have h_div' : GRHStub.OrbitEscapes Collatz.orbit n := by
    simpa [collatz_diverges_eq_orbitEscapes n] using h_div
  have h_roll : GRHStub.hasProjectionRollover collatz_projection_residue Collatz.orbit n := by
    refine ⟨j, k, h_jk, ?_⟩
    simp [GRHStub.endpointProjectionGap] at *
    linarith
  exact GRHStub.projection_rollover_blocks_escape collatz_projection_residue Collatz.orbit n h_roll h_div'

abbrev CollatzGRH : Prop :=
  GRHStub.criticalLineHypothesis CollatzLFunction

abbrev GRH_Extended : Prop := GRHStub.extended CollatzGRH

theorem grh_of_extended (h : GRH_Extended) : GRH := h.1
theorem collatz_grh_of_extended (h : GRH_Extended) : CollatzGRH :=
  GRHStub.payload_of_extended h
theorem grh_extended_implies_rh (h : GRH_Extended) : RiemannHypothesis :=
  GRHStub.extended_implies_rh h

/-- Remaining global divergence claim, now expressed through the imported
    GRH residue story. -/
theorem helix_no_divergence (n : Nat) (h_n : n > 4) :
    ¬ Collatz.diverges n := by
  intro h_div
  obtain ⟨j, k, h_jk, h_res⟩ := escape_forces_projection_rollover n h_n h_div
  exact projection_rollover_blocks_divergence n j k h_jk h_res h_div
...
theorem orbit_injective_of_baker
    ...
    Collatz.orbit n k ≠ Collatz.orbit n j := by
  have h_res : accumulated_projection_loss n k ≥ accumulated_projection_loss n j + 1 := by
    rwa [accumulated_projection_loss_eq_accumulated_offset n k,
      accumulated_projection_loss_eq_accumulated_offset n j]
  exact orbit_injective_of_projection_loss n j k h_res
...
/-- The 3D structure is canonical. When the accumulated projection loss
    ...
    orbit unrealizable. Same theorem, same geometry, same consequence. -/
theorem helix_theorem :
  (∀ (n j k : Nat),
    accumulated_offset n k ≥ accumulated_offset n j + 1 →
    Collatz.orbit n k ≠ Collatz.orbit n j) ∧
  (∀ (n : Nat), n > 4 → ¬ Collatz.diverges n) := by
  constructor
  · intro n j k h_off
    exact orbit_injective_of_baker n j k h_off
  · exact helix_no_divergence
